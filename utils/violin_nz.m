%
function[h,L,MX,MED,bw]=violin_nz(Y,varargin)
...
if isempty(find(strcmp(varargin,'plotdata')))==0
    plotdata = varargin{find(strcmp(varargin,'plotdata'))+1};
end
...
for i=i:size(Y,2)
            fy = F(:,i); ux = U(:,i);
%         fy(ux<-.02)=[]; ux(ux<-.02)=[];
                ym = min(Y{i}); fy(ux<ym-.1) = []; ux(ux<ym-.1) = [];

    if isempty(lc) == 1
        if setX == 0
            h(i)=fill([fy+i;flipud(i-fy)],[U(:,i);flipud(ux)],fy,'FaceAlpha',alp,'EdgeColor','none');
        else
            h(i)=fill([fy+x(i);flipud(x(i)-fy)],[ux;flipud(ux)],fc(i,:),'FaceAlpha',alp,'EdgeColor','none');
        end
    else
        if setX == 0
            h(i)=fill([fy+i;flipud(i-fy)],[ux;flipud(ux)],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        else
            h(i)=fill([fy+x(i);flipud(x(i)-fy)],[ux;flipud(ux)],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        end
    end
    hold on
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(ux,fy+i,MX(:,i)), interp1(flipud(ux),flipud(i-fy),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'LineWidth',1.5);
        end
        if plotmedian == 1
            p(2)=plot([interp1(ux,fy+i,MED(:,i)), interp1(flipud(ux),flipud(i-fy),MED(:,i)) ],[MED(:,i) MED(:,i)],medc,'LineWidth',1.5);
        end
        if plotdata == 1
            p(3)=plot(i+(rand(size(Y{i}))-.5)*.15,Y{i},'.','markersize',6,...
                'color',fc(i,:));
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(ux,fy+i,MX(:,i))+x(i)-i, interp1(flipud(ux),flipud(i-fy),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)],mc,'LineWidth',1.5);
        end
        if plotmedian == 1
            p(2)=plot([interp1(ux,fy+i,MED(:,i))+x(i)-i, interp1(flipud(ux),flipud(i-fy),MED(:,i))+x(i)-i],[MED(:,i) MED(:,i)],medc,'LineWidth',1.5);
        end
        if plotdata == 1
            p(3)=plot(x(i)+(rand(size(Y{i}))-.5)*.15,Y{i},'.','markersize',6,...
                'color',fc(i,:));
%         o
        end
    end
