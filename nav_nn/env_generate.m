...
clear f_rad f_loc
cfdist = [10 500 1500 1500]; % expected distance of cue source from center of food patch, negative indicates uniform distribution across arena
ecf = [5 15 75 2000]; % error on cfdist
np = 5; % number of food patches
f_loc{1} = [(rand(np,1)-.5)*xrn (rand(np,1)-.5)*yrn];
f_rad = 50*ones(1,num_f); % food patch radii
nsc = [np*ones(1,4)]; % number of cue sources, negative indicates uniform distribution
c_rad = [200*ones(1,4)]; % width of the cue distributions, inf indicates uniform
...
env_feat = fcdist_gen_2(cpm,xgrid,ygrid,pbl);
